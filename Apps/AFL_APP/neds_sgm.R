...
if (nrow(neds_sgm_raw) > 0 && "match" %in% names(neds_sgm_raw)) {
  neds_sgm_over <- neds_sgm_raw |>
    ...
    transmute(match = .data$match,
              ...
              entrant_id = .data$entrant_id) |>
    distinct(player_name, market_name, line, type, .keep_all = TRUE)

  neds_sgm_under <- tibble()
  if ("under_price" %in% names(neds_sgm_raw)) {
    neds_sgm_under <- neds_sgm_raw |>
  ...
  neds_sgm <- bind_rows(neds_sgm_over, neds_sgm_under)
} else {
  neds_sgm <- tibble(
    match = character(),
...
call_sgm_neds <- function(data, player_names, stat_counts, markets, types) {
